...
def is_publicly_verified(record: Dict[str, Any]) -> bool:
    """Public badge authority: verified only when lifecycle is current."""
    summary = verification_lifecycle_summary(record)
    status = str(summary.get("status") or summary.get("lifecycle_status") or "").strip().lower()
