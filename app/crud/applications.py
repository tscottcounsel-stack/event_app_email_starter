...
def create_application(db: Session, vendor_id: int, payload: ApplicationCreate) -> Application:
    ...
    if not v:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Vendor profile required before applying")

    existing = db.query(Application).filter(
        Application.event_id == payload.event_id,
...
def list_by_event(db: Session, event_id: int) -> List[Application]:
    return db.query(Application).filter(Application.event_id == event_id).order_by(Application.created_at.asc()).all()
...
def vendor_update_app(
    ...
    if app.vendor_id != getattr(user, "vendor_id", None): raise HTTPException(403, "Forbidden")
    if app.status != "submitted": raise HTTPException(409, "Cannot edit after review started")
    return update_vendor_fields(db, app, body)

def update_vendor_fields(db: Session, app: Application, body):
    if body.note is not None: app.note = body.note
    if body.price_cents is not None: app.price_cents = body.price_cents
    db.add(app); db.commit(); db.refresh(app)
    return app
