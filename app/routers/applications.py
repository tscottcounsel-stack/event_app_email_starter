...
def update_application_progress(
    ...
    if payload.booth_id is not None:
        # Prevent booth changes after payment
        ...
        booth_id = str(payload.booth_id or "").strip()
        ...
        if booth_id:
            ...
            if conflict:
                raise HTTPException(
                    status_code=409,
                    detail="Booth is not available (reserved or occupied).",
                )

            app["booth_id"] = booth_id
            app["booth_reserved_until"] = (
...
def submit_application(
    ...
    booth = app.get("booth_id")

    # Compliance check
    compliance_complete = bool(checked) and all(bool(v) for v in checked.values())

    # Document check
    docs_complete = bool(docs) and all(bool(v) for v in docs.values())

    # Booth check
    booth_selected = bool(booth)

    if not compliance_complete or not docs_complete or not booth_selected:
        raise HTTPException(
            status_code=400,
            detail="Application requirements incomplete. Complete compliance items, upload documents, and select a booth before submitting."
        )
...
def organizer_reserve_booth(app_id: int, body: ReserveBoothBody = Body(...)):
    ...
    existing = [
        a for a in _APPLICATIONS.values()
        if int(a.get("event_id") or 0) == int(event_id)
        and _norm_email(a.get("vendor_email")) == _norm_email(app.get("vendor_email"))
        and str(a.get("booth_id") or "").strip()
        and int(a.get("id") or 0) != int(app_id)
        and _coerce_payment_status(a.get("payment_status")) in ("unpaid", "pending", "paid")
    ]
    if existing:
        raise HTTPException(
            status_code=409,
            detail="Vendor already has a reserved booth for this event.",
        )
    hold_hours = int(body.hold_hours or 24)
...
def organizer_extend_reservation(
    ...
    extend_hours = int(body.extend_hours or 24)
...
def mark_application_paid(
    ...
    email = _norm_email(user.get("email"))
    app_email = _norm_email(app.get("vendor_email"))
    ...
    if not app.get("booth_id"):
        raise HTTPException(
            status_code=400,
            detail="Cannot mark paid without a booth selection.",
        )
