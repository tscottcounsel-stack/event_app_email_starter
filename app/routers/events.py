...
logger.warning("🔥 app.routers.events loaded (postgres events only)")
...
def _owned_events_for_user(db: Session, user: Dict[str, Any]) -> list[Event]:
    ...
    return [e for e in rows if _event_belongs_to_user(_serialize_event_model(e), user)]
...
def organizer_earnings(
    ...
    owned_event_ids = {int(e.id or 0) for e in _owned_events_for_user(db, user)}
