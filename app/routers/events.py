...
def organizer_put_requirements(
    event_id: int, payload: Dict[str, Any], user: dict = Depends(get_current_user)
):
    _get_owned_event_or_404(event_id, user)

    normalized = _normalize_public_requirements_payload(payload or {})
    _REQUIREMENTS[int(event_id)] = normalized

    ev = _EVENTS.get(int(event_id))
    if isinstance(ev, dict):
        ev["requirements"] = normalized
        ev["requirements_updated_at"] = normalized.get("updated_at") or utc_now_iso()
        ev["updated_at"] = utc_now_iso()
