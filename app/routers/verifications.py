...
def _verification_status_rank(value: Any) -> int:
    status = _safe_lower(value)
    if status in {"verified", "approved"}:
        return 50
    if status == "expiring_soon":
        return 45
    if status in {"expired", "needs_renewal", "renewal_pending"}:
        return 40
    if status == "pending":
        return 30
    if status == "rejected":
        return 20
    if status in {"not_started", "unverified"}:
        return 10
    return 0


def _verification_identity_key(record: Dict[str, Any]) -> str:
    return f"{_safe_lower(record.get('role'))}:{_safe_lower(record.get('email'))}"


def _get_verification_record_by_id(verification_id: Any) -> tuple[Any, Optional[Dict[str, Any]]]:
    # JSON object keys may reload as strings even when we originally saved ints.
    # Support both so approval/rejection does not silently fail or leave stale pending rows.
...
def _apply_profile_truth(public: Dict[str, Any], profile: Optional[Profile]) -> Dict[str, Any]:
    if profile is None:
        return public
...
def get_admin_verifications(db: Session = Depends(get_db)):
    """Return a de-duplicated admin verification queue using Postgres Profile as truth.

    The JSON store still carries submitted documents/payment metadata, but it can
    contain stale duplicate pending rows after redeploys or resubmissions. This
    endpoint collapses records by role+email and lets Profile override status so
    verified users do not reappear as pending.
    """
    profile_by_key: Dict[str, Profile] = {}
    for profile in db.query(Profile).all():
        key = f"{_safe_lower(profile.role)}:{_safe_lower(profile.email)}"
        if key != ":":
    ...
    for raw_record in _VERIFICATIONS.values():
        ...
        existing_rank = _verification_status_rank(
            existing.get("status") or existing.get("verification_status") or existing.get("review_status")
        )
        public_rank = _verification_status_rank(
            public.get("status") or public.get("verification_status") or public.get("review_status")
        )
        ...
        if public_rank > existing_rank or (public_rank == existing_rank and public_time > existing_time):
            # Keep documents from both rows when possible so old uploads do not vanish.
            existing_docs = existing.get("documents") if isinstance(existing.get("documents"), list) else []
            public_docs = public.get("documents") if isinstance(public.get("documents"), list) else []
            seen_urls = set()
            merged_docs = []
            for doc in [*public_docs, *existing_docs]:
                if not isinstance(doc, dict):
                    continue
                marker = _safe_str(doc.get("url") or doc.get("name") or doc.get("label"))
                if marker and marker in seen_urls:
                    continue
                if marker:
                    seen_urls.add(marker)
                merged_docs.append(doc)
            public["documents"] = merged_docs
            by_identity[key] = public
        else:
            existing_docs = existing.get("documents") if isinstance(existing.get("documents"), list) else []
            public_docs = public.get("documents") if isinstance(public.get("documents"), list) else []
            if len(public_docs) > len(existing_docs):
                existing["documents"] = public_docs
...
def review_verification(
    ...
    # Mirror the review result to duplicate stale records for the same email/role
    # so old pending rows do not return after refresh or redeploy.
    ...
    for other in _VERIFICATIONS.values():
        ...
        if _safe_lower(other.get("email")) == reviewed_email and _safe_lower(other.get("role")) == reviewed_role:
            ...
            other["last_verified_at"] = record.get("last_verified_at")
            other["expires_at"] = record.get("expires_at")
            other["expiration_date"] = record.get("expiration_date")
            other["locked"] = True
