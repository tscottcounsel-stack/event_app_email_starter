...
Write-Host "🚀 Starting serial API test against $BaseUrl..."
...
function Safe-Request {
    ...
    try {
        ...
        if ($_.Exception.Response -and $_.Exception.Response.Content) {
            Write-Host "⚠ Raw Response: $($_.Exception.Response.Content)" -ForegroundColor DarkGray
        }
        return $null
    }
}
...
if ($userResp) {
    ...
    Write-Host "✅ User created with ID: $($user.id) ($($user.email))" -ForegroundColor Green
}
...
if ($orgResp) {
    ...
    Write-Host "✅ Organizer created with ID: $($organizer.id)" -ForegroundColor Green
}
...
if ($vendorResp) {
    ...
    Write-Host "✅ Vendor created with ID: $($vendor.id)" -ForegroundColor Green
}
...
if ($event -and $vendor) {
    ...
    if ($appResp) {
        ...
        Write-Host "✅ Application created with ID: $($application.id) (status: $($application.status))" -ForegroundColor Green
    }
}
...
if ($application) {
    ...
    Write-Host "🗑 Application $($application.id) deleted" -ForegroundColor Yellow
}
if ($event) {
    Safe-Request "$BaseUrl/events/$($event.id)" -Method DELETE | Out-Null
    Write-Host "🗑 Event $($event.id) deleted" -ForegroundColor Yellow
}
if ($vendor) {
    Safe-Request "$BaseUrl/vendors/$($vendor.id)" -Method DELETE | Out-Null
    Write-Host "🗑 Vendor $($vendor.id) deleted" -ForegroundColor Yellow
}
if ($organizer) {
    Safe-Request "$BaseUrl/organizers/$($organizer.id)" -Method DELETE | Out-Null
    Write-Host "🗑 Organizer $($organizer.id) deleted" -ForegroundColor Yellow
}
if ($user) {
    Safe-Request "$BaseUrl/users/$($user.id)" -Method DELETE | Out-Null
    Write-Host "🗑 User $($user.id) deleted" -ForegroundColor Yellow
}

Write-Host "`n🎉 Serial test complete!" -ForegroundColor Green
