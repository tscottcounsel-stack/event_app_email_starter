...
def test_event_creation_unauthorized(base_url, json_headers):
    ...
    res = requests.post(
        ...
        data=json.dumps({"title":"Unauthorized Event","description":"oops","date":start,"location":"Nowhere"}),
        timeout=10,
    )
...
async def test_event_flow():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        ev_res = await ac.post("/events/", json=event_data,
        ...
        reg_v = await ac.post("/auth/register", json={
            "email": vendor_email,
            "password": "testpass",
            "role": "vendor"
        })
        ...
        login_v = await ac.post("/auth/login", json={
            "email": vendor_email,
            "password": "testpass"
        })
        ...
        app_res = await ac.post("/applications/", json={
            "event_id": event_id,
        ...
        list_res = await ac.get(f"/applications/event/{event_id}",
                                headers={"Authorization": f"Bearer {org_token}"})
        ...
        upd_res = await ac.put(f"/applications/{app_id}?status=approved",
                               headers={"Authorization": f"Bearer {org_token}"})
        ...
        my_apps = await ac.get("/applications/mine",
                               headers={"Authorization": f"Bearer {vendor_token}"})
