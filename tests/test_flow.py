...
def test_register_login_and_crud(test_client: TestClient):
    ...
    r = test_client.post("/auth/register", json={
        "email": "v1@example.com", "password": "secret123", "role": "vendor"
    })
    assert r.status_code == 200

    # Login vendor
    r = test_client.post("/auth/login", json={
        "email": "v1@example.com", "password": "secret123"
    })
    assert r.status_code == 200
    tok = r.json()
    ...
    r = test_client.post("/vendors/", json={"display_name": "Vendor One", "bio": "hi"}, headers=headers)
    ...
    # Register organizer
    r = test_client.post("/auth/register", json={
        "email": "o1@example.com", "password": "secret123", "role": "organizer"
    })
    assert r.status_code == 200

    # Login organizer
    r = test_client.post("/auth/login", json={"email": "o1@example.com", "password": "secret123"})
    org_tok = r.json()
    org_headers = {"Authorization": f"Bearer {org_tok['access_token']}"}

    # Create event
    now = datetime.now(timezone.utc)
    r = test_client.post("/events/", json={
        "title": "Kickoff",
        "description": "demo",
