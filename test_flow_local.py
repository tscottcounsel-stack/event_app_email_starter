...
def run_flow():
    ...
    r = requests.post(
        f"{BASE_URL}/events/", json=event_data, headers=auth_header(organizer_token)
    )
    r.raise_for_status()
    event = r.json()
    event_id = event["id"]
    ...
    for v_email in vendors:
        ...
        print(
            f"Vendor {v_email} applied (application id={application['id']}, status={application['status']})"
        )

    # 4. Organizer views all applications
    r = requests.get(
        f"{BASE_URL}/applications/event/{event_id}",
    ...
    for i, app_id in enumerate(app_ids):
        ...
        r = requests.put(
            f"{BASE_URL}/applications/{app_id}?status={status}",
            headers=auth_header(organizer_token),
        )
        r.raise_for_status()
        updated = r.json()
    ...
    for v_email in vendors:
        vendor_token = login(v_email, password)
        r = requests.get(
            f"{BASE_URL}/applications/mine", headers=auth_header(vendor_token)
        )
        r.raise_for_status()
        vendor_apps = r.json()
