...
from models import User, UserRole, VendorProfile, OrganizerProfile
from schemas import (
    VendorProfileIn, VendorProfileOut,
    OrganizerProfileIn, OrganizerProfileOut
)
...
def get_my_organizer_profile(
    ...
    prof = db.query(OrganizerProfile).filter(OrganizerProfile.user_id == current.id).first()
    if not prof:
