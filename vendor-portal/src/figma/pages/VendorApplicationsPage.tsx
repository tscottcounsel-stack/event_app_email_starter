...
export default function VendorApplicationsPage() {
 ...
 useEffect(() => {
  ...
  console.log("PAYMENT DEBUG:", { payment, appId, sessionId });

  setPaymentMessage(`DEBUG → payment=${payment} appId=${appId} session=${sessionId}`);

  if (payment !== "success") return;
  if (!appId) return;
  if (!sessionId) return;

  console.log("CALLING CONFIRM PAYMENT");

  fetch(`${API_BASE}/vendor/applications/${appId}/confirm-payment`, {
    method: "POST",
    headers: {
      ...buildAuthHeaders(),
      "Content-Type": "application/json",
    ...
    .then(async (res) => {
      ...
      if (!res.ok) {
        throw new Error(data?.detail || `Unable to confirm payment (${res.status}).`);
      }
    ...
    .catch((err) => {
      ...
      setPaymentMessage(`❌ ${err?.message || "Unable to confirm payment."}`);
    });
}, [loadApplications]);
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-4xl font-black tracking-tight text-slate-900"> Applications TEST 123</h1>
