...
function pathMatchesRole(fromPath: string, role: Role) {
...
export default function PublicLoginPage() {
  ...
  // IMPORTANT: keep a ref so submit always uses the latest selected role
  ...
  function setRoleAndDefaults(next: Role) {
    ...
    // Optional: auto-fill demo accounts
    setEmail(next === "organizer" ? "organizer@example.com" : "vendor@example.com");
  ...
  async function handleContinue(e?: React.FormEvent | React.MouseEvent) {
    ...
    try {
      ...
      // Return-to route (from RequireAuth), but only if it matches the role selected.
      const from = (location.state as any)?.from;
      const fallback = roleHome(submitRole);
      const dest = typeof from === "string" && pathMatchesRole(from, submitRole) ? from : fallback;

      // Let auth state settle before route guards evaluate
      setTimeout(() => {
