...
import { apiGet, type OrganizerEventListItem, API_BASE } from "../api";
...
export default function OrganizerDashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const draft = items.filter(
      (e: any) => String(e?.status || "").toLowerCase() === "draft"
    ).length;

    const pendingApps = 0; // placeholder until endpoint exists
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            A simple overview of your events and activity.
          </p>
        </div>
      ...
      {/* Your Events — now matches Events page exactly */}
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="mt-3 flex flex-wrap gap-3 text-sm sm:mt-0">
                    ...
                    <button
                      ...
                      onClick={() =>
                        nav(`/organizer/applications?eventId=${e.id}`)
                      }
                    >
                      Applications
                    </button>
